...
>
> **Última actualización:** 2026-09-02 (sesión "auditoría por mutación: rotación, tests intermitentes y arranque") · **Estado:** 🚀 **Beta abierta a usuarios** · **Tests:** 672 pasando (52 archivos, ~9.400 líneas de test) · **LOC:** ~13.300 JS + ~2.725 CSS + ~600 HTML.
>
...
>
> ## 🧗 TRES EJERCICIOS DE POTENCIA PARA INTERMEDIOS + TOPE DE CAMPUS (2026-09-02)
>
> El arreglo de fondo de la deuda que dejó la auditoría: **el algoritmo repartía mejor, pero no podía inventar estímulos**. Un intermedio tenía 5 ejercicios de potencia para 9 huecos semanales. Ahora tiene **8**, y son los que le faltaban: el escalón entre la pared y el campus board.
>
> | id | Qué es | cat / tier |
> |---|---|---|
> | `pow2b` | **Campus con pies (escalera asistida)** — escaleras mano a mano con los pies apoyados | `campus_board` / 1 |
> | `pow2c` | **Bloque sin pies en presas grandes** — 3-5 movimientos colgando sólo de las manos, sobre presas muy buenas | `power` / 1 |
> | `pow2d` | **Lanzamientos progresivos con impulso de piernas** — el mismo lanzamiento tocando cada vez más arriba | `wall_training` / 1 |
>
> Los tres siguen la progresión publicada de carga sobre la polea A2 —**pies apoyados → un pie → sin pies**— y ninguno se hace en arqueo completo, que combinado con movimiento dinámico es donde se rompen las poleas. `pow2b` cubre una categoría que para un intermedio estaba **vacía**: el primer hueco de cada sesión de potencia pide `campus_board` y no había un solo candidato de su nivel, así que ese hueco nunca se llenaba con lo que pedía.
>
> **Dos hallazgos que salieron de escribirlos** (ninguno buscado):
>
> 1. **La app cumplía el límite de campus por casualidad.** La recomendación publicada es **máximo 2 sesiones de campus por semana, con 48-72 h entre ellas**; el código no la conocía: la categoría competía de igual a igual con el resto y decidía la semilla. Bastaba agregar ejercicios al pool para empezar a incumplirla sin que nada avisara. Ahora hay un tope explícito (`MAX_SESIONES_CAMPUS`, cuenta sesiones y también el campus con pies, por conservador) y un test que lo custodia.
> 2. **La otra cara: un AVANZADO con tabla pasaba un ciclo entero de 10 semanas con CERO campus.** `SLOT_COMPOSITION` lista `['campus_board','power']` y el comentario dice "campus si el nivel lo permite", pero el código leía la lista como un **conjunto plano**: el criterio estaba en el comentario, no en el código. Ahora se respeta el orden de preferencia. Verificado en 4 perfiles: campus aparece, y nunca más de 2 veces por semana.
>
> *Un test viejo tuvo que cambiar, y vale como ejemplo:* `gear.test.js` exigía que **todo** sustituto de campus hablara de "sin pies" — cierto mientras los tres ejercicios de campus eran sin pies, falso desde `pow2b`. La regla correcta no era "sin pies" sino **conservar la condición de apoyo**: dar un sustituto sin pies a un ejercicio con pies no es adaptar, es cambiar el ejercicio por otro más cargado para los dedos.
>
...
2. ✅ ~~Más pool de potencia para intermedios~~ — **HECHO**: `pow2b`/`pow2c`/`pow2d` suben el pool de tier 1 de 5 a 8 y llenan la categoría `campus_board`, que estaba vacía para ese nivel (ver el bloque 🧗). *Queda el mismo ejercicio para otros bloques*: revisar si `strength` (7 disponibles a tier 1) y las categorías de un solo ejercicio necesitan la misma cura.
