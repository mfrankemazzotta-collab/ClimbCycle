...
function renderProfile(){
  ...
  var zones=[
    ...
    {z:2,n:'Base Aeróbica',   min:arMin,                    max:arMax,     c:'#CCFF00'},
  ...
  h+='<div class="sec">Zona Óptima de Arousal</div><div class="card"><div style="font-size:10px;color:var(--text-muted);font-family:\'JetBrains Mono\',monospace;margin-bottom:12px">FCmax = '+maxHR+' bpm (Lach 2021)</div><div style="background:var(--accent-primary-bg);border:1.5px solid #CCFF00;border-radius:12px;padding:14px;margin-bottom:14px"><div style="font-size:10px;color:var(--accent-primary-d);font-family:\'JetBrains Mono\',monospace;margin-bottom:4px">ZONA ÓPTIMA</div><div style="font-family:\'JetBrains Mono\',monospace;font-size:28px;font-weight:700;color:var(--accent-primary-d)">'+arMin+' - '+arMax+' bpm</div><div style="font-size:11px;color:var(--text-secondary);margin-top:4px">60-70% FC Reserva - Karvonen - Arent & Landers (2003)</div></div>'+zones.map(function(z,i){return '<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:'+(i<zones.length-1?'1px solid var(--border-color)':'none')+'"><div style="width:10px;height:10px;border-radius:50%;background:'+z.c+';flex-shrink:0"></div><div style="flex:1;font-size:12px;font-weight:600;color:'+(z.z===2?'#CCFF00':'var(--text-primary)')+'">Z'+z.z+' '+z.n+(z.z===2?' [ÓPTIMA]':'')+'</div><div style="font-family:\'JetBrains Mono\',monospace;font-size:11px;color:var(--text-secondary)">'+z.min+'-'+z.max+'</div></div>';}).join('')+'</div>';
  ...
  h+='<div style="margin-top:16px"><button onclick="if(confirm(\'Reiniciar y borrar todo tu plan actual?\'))location.reload()" style="width:100%;padding:12px;background:none;border:1px solid var(--border-color);border-radius:10px;color:var(--text-muted);font-size:12px;cursor:pointer">Reiniciar y crear nuevo plan</button></div>';
  /* User info + logout */
  if(typeof currentUser !== 'undefined' && currentUser){
    h+='<div style="margin-top:24px;padding:14px;background:var(--bg-card);border:1px solid var(--border-color);border-radius:12px">'
      ...
      +'<div style="display:flex;justify-content:space-between;align-items:center">'
        ...
        +'<button onclick="if(confirm(\'Cerrar sesión?\'))logoutUser()" style="padding:8px 14px;background:none;border:1px solid #FF4D6A55;border-radius:8px;color:#FF4D6A;font-size:11px;font-family:\'JetBrains Mono\',monospace;cursor:pointer;touch-action:manipulation">Cerrar sesión</button>'
      +'</div>'
