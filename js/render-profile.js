...
function renderProfile(){
  ...
  var zones=[
    {z:1,n:'Recuperacion',    min:Math.round(res*.50+U.rhr),max:arMin-1,   c:'var(--text-muted)'},
  ...
  h+='<div class="sec">Zona Optima de Arousal</div><div class="card"><div style="font-size:10px;color:var(--text-muted);font-family:\'JetBrains Mono\',monospace;margin-bottom:12px">FCmax = '+maxHR+' bpm (Lach 2021)</div><div style="background:var(--accent-primary-bg);border:1.5px solid #CCFF00;border-radius:12px;padding:14px;margin-bottom:14px"><div style="font-size:10px;color:#CCFF00;font-family:\'JetBrains Mono\',monospace;margin-bottom:4px">ZONA OPTIMA</div><div style="font-family:\'JetBrains Mono\',monospace;font-size:28px;font-weight:700;color:#CCFF00">'+arMin+' - '+arMax+' bpm</div><div style="font-size:11px;color:var(--text-secondary);margin-top:4px">60-70% FC Reserva - Karvonen - Arent & Landers (2003)</div></div>'+zones.map(function(z,i){return '<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:'+(i<zones.length-1?'1px solid var(--border-color)':'none')+'"><div style="width:10px;height:10px;border-radius:50%;background:'+z.c+';flex-shrink:0"></div><div style="flex:1;font-size:12px;font-weight:600;color:'+(z.z===2?'#CCFF00':'var(--text-primary)')+'">Z'+z.z+' '+z.n+(z.z===2?' [OPTIMA]':'')+'</div><div style="font-family:\'JetBrains Mono\',monospace;font-size:11px;color:var(--text-secondary)">'+z.min+'-'+z.max+'</div></div>';}).join('')+'</div>';
  ...
  h+='<div style="margin-top:8px"><button onclick="if(confirm(\'Reiniciar?\'))location.reload()" style="width:100%;padding:12px;background:none;border:1px solid var(--border-color);border-radius:10px;color:var(--text-muted);font-size:12px;cursor:pointer">Reiniciar y crear nuevo plan</button></div>';
  /* User info + logout */
  if(typeof currentUser !== 'undefined' && currentUser){
    h+='<div style="margin-top:24px;padding:14px;background:var(--bg-card);border:1px solid var(--border-color);border-radius:12px">'
      +'<div style="font-family:\'JetBrains Mono\',monospace;font-size:10px;color:var(--text-secondary);text-transform:uppercase;letter-spacing:1px;margin-bottom:6px">Sesion activa</div>'
