...
/* ── Registry ─────────────────────────────────────── */
var WIDGET_DEFS = [
  /* ── Rediseño: Inicio ORIENTA, no ejecuta ──────────────
     Puente de una línea hacia la tab Hoy. Reemplaza a los 3 widgets que
     duplicaban la sesión (próxima acción / hoy / lista del día), que eran la
     fuente principal del abrume. */
  { id:'bridge', title:'Ir a la sesión de hoy', icon:'🎯', group:'', html:function(){
      return '<div id="hoy-bridge"></div>';
  }},
  /* El macrociclo es el hero del hub: un segmento por semana, color por fase. */
  { id:'macro', title:'Mi macrociclo', icon:'📈', group:'estado', html:function(){
      return '<div id="macro-hero"></div>';
  }},
  /* Accesos rápidos: lo que se abre con los dedos entizados. */
  { id:'access', title:'Accesos rápidos', icon:'⚡', group:'accesos', html:function(){
      return '<div id="quick-access"></div>';
  }},
  { id:'glance', title:'Resumen rápido', icon:'⚡', def:false, html:function(){
      return '<div class="glance-grid" id="glance-grid"></div>';
  }},
  { id:'next', title:'Próxima acción', icon:'👉', def:false, html:function(){
      return '<div id="next-action"></div>';
  }},
  { id:'today', title:'Hoy: qué entrenar', icon:'🎯', def:false, html:function(){
      return '<div id="hero-today"></div>';
  }},
  { id:'recovery', title:'Recuperación', icon:'🔋', group:'estado', html:function(){
      return ''
  ...
  { id:'goal', title:'Objetivo', icon:'🏁', group:'seguimiento', html:function(){
      return '<div id="goal-card"></div>';
  }},
  { id:'stats', title:'Progreso de la semana', icon:'📊', group:'seguimiento', html:function(){
      return ''
  ...
  { id:'plan', title:'Calendario del plan', icon:'📅', group:'plan', html:function(){
      return ''
        ...
        + '</div>';
        /* El panel del día (con TODOS los ejercicios) se sacó del hub a
           propósito: eso es ejecución y vive en la tab Hoy. Inicio orienta. */
  }},
  { id:'todaylist', title:'Sesión de hoy (detalle)', icon:'📋', def:false, html:function(){
  ...
  { id:'projects', title:'Proyectos', icon:'📌', group:'seguimiento', html:function(){
      return '<div id="projects-body"></div>';
  }},
  { id:'timer', title:'Temporizador', icon:'⏱️', def:false, html:function(){
...
function loadWidgetConfig(){
  ...
  /* Migración única al rediseño: una config guardada ANTES del rediseño no
     conoce 'bridge'. Si sólo mergeáramos, el usuario quedaría con los widgets
     que duplican la sesión encendidos + los nuevos = más abrume, no menos.
     Detectamos ese caso y adoptamos el layout nuevo (se puede repersonalizar). */
  var conoceRediseno = saved.some(function(w){ return w && w.id === 'bridge'; });
  if(!conoceRediseno){
    var cfg = defaultWidgetConfig();
...
function renderWidgets(){
  ...
  if(typeof renderInstallPrompt === 'function'){ try { renderInstallPrompt(); } catch(e){} }
  ...
  var html = '';
  /* Orden fijo de secciones: primero "dónde estoy", después "qué uso en el
     gimnasio", después lo que se mira 1 vez por semana. Los widgets sin grupo
     (o group:'') van arriba, sin encabezado. */
  var GRUPOS = [
    { id:'',            label:'' },
  ...
  GRUPOS.forEach(function(g){
    var enGrupo = cfg.filter(function(w){
      var def = WIDGET_DEFS_BYID[w.id];
      return w.on && def && (def.group || '') === g.id;
    });
    if(!enGrupo.length) return;
    if(g.label){
      html += '<div style="font-family:\'JetBrains Mono\',monospace;font-size:11px;font-weight:700;letter-spacing:1.2px;text-transform:uppercase;color:var(--text-muted);margin:18px 0 8px">'+g.label+'</div>';
    }
    enGrupo.forEach(function(w){
      html += '<div class="widget" data-wid="' + w.id + '">' + WIDGET_DEFS_BYID[w.id].html() + '</div>';
    });
  });
  host.innerHTML = html;
  populateWidgets();
...
/* ── Rediseño: los 3 bloques nuevos del hub ──────────── */

/* Puente de UNA línea hacia la tab Hoy. Inicio orienta, Hoy ejecuta. */
function renderHoyBridge(){
  var el = document.getElementById('hoy-bridge'); if(!el) return;
  if(!U.startDate){ el.innerHTML = ''; return; }
  var key  = TODAY.toDateString();
  var plan = planMap[key];
  var esDescanso = !plan || plan.block === 'rest';
  var bt   = (!esDescanso && BLOCKS[plan.block]) || null;
  var col  = bt ? bt.col : 'var(--accent-deload)';
  var exs  = (!esDescanso && typeof getExercisesForDay === 'function') ? getExercisesForDay(key, plan.block) : [];
  var nDone= (typeof countExDone === 'function') ? countExDone(key, exs) : 0;
  if(sessionLog[key] === 'done') nDone = exs.length;
  var sub  = esDescanso
    ? (plan && plan.outdoor ? 'Día de roca' : 'Descanso &middot; el plan lo necesita')
    : exs.length + ' ejercicio' + (exs.length===1?'':'s') + ' &middot; ' + (U.session||90) + ' min &middot; <strong style="color:var(--text-primary)">' + nDone + '/' + exs.length + ' hechos</strong>';
  el.innerHTML = '<button onclick="goPage(\'hoy\')" style="width:100%;text-align:left;display:flex;align-items:center;gap:12px;min-height:44px;padding:14px;background:var(--bg-card);border:1px solid var(--border-color);border-left:3px solid '+col+';border-radius:14px;cursor:pointer;touch-action:manipulation">'
    +'<span style="flex:1;min-width:0">'
      +'<span style="display:block;font-family:\'JetBrains Mono\',monospace;font-size:11px;font-weight:700;letter-spacing:1px;text-transform:uppercase;color:'+col+'">Hoy'+(bt?' &middot; '+escapeHtml(bt.label):'')+'</span>'
      +'<span style="display:block;font-size:12px;color:var(--text-secondary);margin-top:3px">'+sub+'</span>'
    +'</span>'
    +'<span style="flex:none;color:var(--text-muted);font-size:20px">&rsaquo;</span>'
  +'</button>';
}

/* Macrociclo: un segmento por semana con el color de su fase. El único gráfico
   del hub — se entiende sin leer. */
function renderMacroHero(){
  var el = document.getElementById('macro-hero'); if(!el) return;
  var seq = (typeof getPlanSeq === 'function') ? getPlanSeq() : null;
  if(!U.startDate || !seq || !seq.length){ el.innerHTML = ''; return; }
  var actual = (typeof getCurrentWeekIndex === 'function') ? getCurrentWeekIndex() : 0;
  var segs = seq.map(function(block, i){
    var b = BLOCKS[block] || {col:'var(--text-muted)'};
    var esActual = (i === actual);
    return '<div title="Semana '+(i+1)+'" style="flex:1;min-width:0;height:30px;border-radius:7px;display:flex;align-items:center;justify-content:center;'
      +'background:color-mix(in srgb, '+b.col+' '+(i<actual?'55':esActual?'100':'22')+'%, transparent);'
      +(esActual?'outline:2px solid var(--text-primary);outline-offset:1px;':'')
      +'font-family:\'JetBrains Mono\',monospace;font-size:11px;font-weight:700;color:'+(esActual?'var(--accent-primary-on)':'var(--text-secondary)')+'">'+(i+1)+'</div>';
  }).join('');
  var vistos = {};
  var leyenda = seq.filter(function(b){ if(vistos[b])return false; vistos[b]=1; return true; }).map(function(b){
    var m = BLOCKS[b] || {label:b, col:'var(--text-muted)'};
    return '<span style="display:inline-flex;align-items:center;gap:5px;font-size:11px;color:var(--text-secondary)"><span style="width:8px;height:8px;border-radius:50%;background:'+m.col+'"></span>'+escapeHtml(m.label)+'</span>';
  }).join('');
  el.innerHTML = '<div class="card" style="padding:14px">'
    +'<div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:10px">'
      +'<span style="font-family:\'JetBrains Mono\',monospace;font-size:11px;font-weight:700;letter-spacing:1.2px;text-transform:uppercase;color:var(--text-muted)">Mi macrociclo</span>'
      +'<span style="font-family:\'JetBrains Mono\',monospace;font-size:12px;color:var(--text-secondary)">Sem <strong style="color:var(--accent-primary-d)">'+(actual+1)+'</strong>/'+seq.length+'</span>'
    +'</div>'
    +'<div style="display:flex;gap:4px">'+segs+'</div>'
    +'<div style="display:flex;flex-wrap:wrap;gap:10px;margin-top:10px">'+leyenda+'</div>'
  +'</div>';
}

/* Accesos rápidos: lo que se abre en el gimnasio, con área táctil generosa. */
function renderQuickAccess(){
  var el = document.getElementById('quick-access'); if(!el) return;
  var tile = function(onclick, icon, label){
    return '<button onclick="'+onclick+'" style="flex:1;min-width:0;min-height:64px;display:flex;align-items:center;gap:10px;padding:12px;background:var(--bg-card);border:1px solid var(--border-color);border-radius:14px;cursor:pointer;touch-action:manipulation;text-align:left">'
      +'<span style="width:38px;height:38px;flex:none;border-radius:10px;background:var(--accent-primary-bg);display:flex;align-items:center;justify-content:center;font-size:18px">'+icon+'</span>'
      +'<span style="flex:1;min-width:0;font-family:\'Barlow Condensed\',sans-serif;font-size:15px;font-weight:700;color:var(--text-primary);line-height:1.15">'+label+'</span>'
    +'</button>';
  };
  el.innerHTML = '<div style="display:flex;gap:10px">'
    + tile('if(typeof openTimer===\'function\')openTimer({})', '&#x23F1;', 'Temporizador')
    + tile('goPage(\'plan\');setTimeout(function(){if(typeof swPT===\'function\')swPT(\'hb\');},100)', '&#x1F590;', 'Protocolos de dedos')
  +'</div>';
}
...
function populateWidgets(){
  ...
  try { if(g('hoy-bridge'))   renderHoyBridge();   } catch(e){ if(typeof logError==='function') logError(e,'renderHoyBridge'); }
  try { if(g('macro-hero'))   renderMacroHero();   } catch(e){ if(typeof logError==='function') logError(e,'renderMacroHero'); }
  try { if(g('quick-access')) renderQuickAccess(); } catch(e){ if(typeof logError==='function') logError(e,'renderQuickAccess'); }
  ...
  /* Solo el calendario: el detalle del día vive en la tab Hoy, no en el hub. */
  try { if(g('hcal-grid') && typeof renderHC === 'function') renderHC(); } catch(e){}
