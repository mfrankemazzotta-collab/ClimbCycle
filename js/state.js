...
var lastExUsed = {};
/* Progreso DENTRO de la sesión: qué ejercicios marcaste ya.
   Forma: { 'Mon Jan 05 2026': { str1:true, str3:true } }
   Se indexa por ID de ejercicio (no por posición) para que siga siendo válido
   aunque la selección se recalcule. Persistido en cc_exdone e incluido en los backups. */
var exDone = {};
var toastTmr = null;
...
function saveExDone(){try{localStorage.setItem('cc_exdone',JSON.stringify(exDone));}catch(e){}}
function loadExDone(){try{var s=localStorage.getItem('cc_exdone');if(s)exDone=JSON.parse(s);}catch(e){}}
/* PURE-ish: marca/desmarca un ejercicio del día y devuelve el nuevo estado. */
function toggleExDone(dateStr, exId){
  if(!dateStr || !exId) return false;
  if(!exDone[dateStr]) exDone[dateStr] = {};
  if(exDone[dateStr][exId]){ delete exDone[dateStr][exId]; }
  else { exDone[dateStr][exId] = true; }
  if(!Object.keys(exDone[dateStr]).length) delete exDone[dateStr];
  return !!(exDone[dateStr] && exDone[dateStr][exId]);
}
function isExDone(dateStr, exId){ return !!(exDone[dateStr] && exDone[dateStr][exId]); }
function countExDone(dateStr, exs){
  if(!exs || !exs.length) return 0;
  var n = 0;
  for(var i=0;i<exs.length;i++){ if(isExDone(dateStr, exs[i].id)) n++; }
  return n;
}
/* Borra el progreso por ejercicio de un día. Hace falta porque `exDone` y
   `sessionLog` son dos estados que se escribían por separado: al deshacer una
   sesión (o marcarla como no realizada) los ejercicios quedaban tildados, o
   sea una sesión "abierta" con todo hecho — un estado que la propia lógica
   considera imposible (marcar el último la cierra sola). */
function clearExDone(dateStr){
  if(!dateStr || !exDone[dateStr]) return false;
  delete exDone[dateStr];
  return true;
}

/* PURE. Días de `exDone` que ya no corresponden a una sesión entrenable —
   típicamente porque el plan se regeneró y ese día pasó a ser descanso.
   Se pasan `map`/`plan` como argumentos para poder testearlo sin globals. */
function staleExDoneKeys(map, plan){
  var out = [];
  Object.keys(map || {}).forEach(function(k){
    var p = plan && plan[k];
    if(!p || p.block === 'rest' || p.block === 'test') out.push(k);
  });
  return out;
}

/* Purga el progreso huérfano. Conserva el de los días que siguen siendo de
   entrenamiento, así regenerar el plan no borra lo que ya hiciste. */
function pruneExDone(){
  var muertos = staleExDoneKeys(exDone, (typeof planMap !== 'undefined') ? planMap : {});
  muertos.forEach(function(k){ delete exDone[k]; });
