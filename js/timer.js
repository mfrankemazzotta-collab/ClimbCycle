...
function tmrClose(){
  ...
  Tmr.plan = null; Tmr.idx = 0; Tmr.remaining = 0;   /* cierra de verdad: no queda mini colgada */
  var o = document.getElementById('tmr-overlay'); if(o) o.style.display = 'none';
  if(typeof tmrRenderMini === 'function') tmrRenderMini();
}
...
function _tmrStepper(label, field, val, unit){
  return '<div style="display:flex;align-items:center;justify-content:space-between;padding:12px 0;border-top:1px solid var(--border-color)">'
    ...
    + '<div style="display:flex;align-items:center;gap:14px">'
      ...
      + '<div style="min-width:56px;text-align:center"><span style="font-family:\'JetBrains Mono\',monospace;font-size:20px;font-weight:700;color:var(--text-primary)">' + val + '</span>' + (unit ? '<span style="font-size:11px;color:var(--text-muted);margin-left:3px">' + unit + '</span>' : '') + '</div>'
...
/* ── Mini-player sticky ────────────────────────────────
   El temporizador sigue corriendo mientras scrolleás o cambiás de pantalla:
   minimizar oculta el overlay pero NO detiene la cuenta. La barra vive sobre
   la nav; un tap la vuelve a abrir a pantalla completa. */
function tmrMinimize(){
  var o = document.getElementById('tmr-overlay');
  if(o) o.style.display = 'none';
  tmrRenderMini();
}
function tmrOpenFull(){
  var o = document.getElementById('tmr-overlay');
  if(o) o.style.display = 'flex';
  tmrRenderMini();          /* oculta la mini mientras el overlay está abierto */
}
/* ¿hay una cuenta viva (corriendo o pausada a mitad)? */
function tmrActivo(){
  return !!(Tmr.plan && Tmr.plan.phases && Tmr.idx < Tmr.plan.phases.length);
}
function tmrRenderMini(){
  var el = document.getElementById('tmr-mini');
  if(!el) return;
  var o = document.getElementById('tmr-overlay');
  var overlayAbierto = !!(o && o.style.display !== 'none');
  if(!tmrActivo() || overlayAbierto){ el.style.display = 'none'; el.innerHTML = ''; return; }

  var ph = Tmr.plan.phases[Tmr.idx];
  var meta = _tmrPhaseMeta(ph);
  var col = meta.col;
  var etiqueta = (Tmr.cfg && Tmr.cfg.label) ? Tmr.cfg.label : 'Temporizador';
  el.style.display = 'flex';
  el.style.borderTop = '2px solid ' + col;
  el.innerHTML =
    '<button onclick="tmrOpenFull()" aria-label="Abrir temporizador" style="flex:1;min-width:0;min-height:44px;display:flex;align-items:center;gap:10px;background:none;border:none;cursor:pointer;text-align:left;padding:0;color:var(--text-primary);touch-action:manipulation">'
      +'<span style="width:8px;height:8px;border-radius:50%;background:'+col+';flex:none'+(Tmr.running?';animation:ccPulse 1.6s ease-in-out infinite':'')+'"></span>'
      +'<span style="flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:11px;color:var(--text-secondary)">'+escapeHtml(etiqueta)+'</span>'
      +'<span style="flex:none;font-family:\'JetBrains Mono\',monospace;font-size:11px;font-weight:700;letter-spacing:.6px;text-transform:uppercase;color:'+col+'">'+escapeHtml(meta.label)+'</span>'
      +'<span style="flex:none;font-family:\'JetBrains Mono\',monospace;font-size:21px;font-weight:700;color:'+col+'">'+fmtMMSS(Tmr.remaining)+'</span>'
    +'</button>'
    +'<button onclick="tmrMiniPlus()" aria-label="Sumar 30 segundos" style="flex:none;min-width:44px;min-height:44px;background:var(--bg-card-alt);border:1px solid var(--border-color);border-radius:10px;color:var(--text-primary);font-family:\'JetBrains Mono\',monospace;font-size:11px;font-weight:700;cursor:pointer;touch-action:manipulation">+30s</button>'
    +'<button onclick="tmrPauseResume();tmrRenderMini()" aria-label="'+(Tmr.running?'Pausar':'Seguir')+'" style="flex:none;min-width:44px;min-height:44px;background:var(--bg-card-alt);border:1px solid var(--border-color);border-radius:10px;color:var(--text-primary);cursor:pointer;font-size:13px;touch-action:manipulation">'+(Tmr.running?'&#10073;&#10073;':'&#x25B6;')+'</button>'
    +'<button onclick="tmrClose()" aria-label="Cerrar temporizador" style="flex:none;min-width:44px;min-height:44px;background:none;border:none;color:var(--text-muted);cursor:pointer;font-size:18px;touch-action:manipulation">&times;</button>';
}
/* +30s a la fase en curso, sin perder el resto del plan. */
function tmrMiniPlus(){
  if(!tmrActivo()) return;
  Tmr.remaining += 30;
  var ph = Tmr.plan.phases[Tmr.idx]; if(ph) ph.secs += 30;
  tmrUpdateRun(); tmrRenderMini();
}
  ...
  o.innerHTML =
    '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">'
      ...
      + '<div style="display:flex;gap:8px">'
        /* minimizar: la cuenta sigue viva en la barra sobre la nav */
        + '<button aria-label="Minimizar" onclick="tmrMinimize()" style="width:44px;height:44px;border-radius:50%;border:1px solid var(--border-color);background:var(--bg-card-alt);color:var(--text-secondary);font-size:20px;cursor:pointer;touch-action:manipulation">&minus;</button>'
        + '<button aria-label="Cerrar" onclick="tmrClose()" style="width:44px;height:44px;border-radius:50%;border:1px solid var(--border-color);background:var(--bg-card-alt);color:var(--text-secondary);font-size:18px;cursor:pointer;touch-action:manipulation">&times;</button>'
      + '</div>'
    + '</div>'
