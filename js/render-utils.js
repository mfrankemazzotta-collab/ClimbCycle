...
function showDD(date,plan){
  ...
  var dateStr='<div style="font-family:\'Barlow Condensed\',sans-serif;font-size:20px;font-weight:700;margin-bottom:4px;color:'+bt.col+'">'+bt.label+' <span style="color:#444466;font-size:14px">S'+plan.week+'</span></div>'
    +'<div style="font-size:12px;color:#7070AA;margin-bottom:12px">'+date.toLocaleDateString('es-ES',opts)+'</div>';

  var body='';
  if(plan.block==='rest'){
    var isRock=plan.outdoor;
    body='<div style="font-size:13px;color:#7070AA">'+(isRock?'Escalada exterior. Las sesiones siguientes estan ajustadas.':'Dia de descanso. Prioriza sueno, hidratacion y nutricion.')+'</div>';
  } else if(plan.block==='test'){
    body='<div style="font-size:13px;color:#7070AA">Tests programados: '+(U.tests&&U.tests.length?U.tests.join(', '):'ninguno seleccionado')+'</div>';
  } else {
    ...
    body+=exs.map(function(e){
      return '<div style="background:#0F0F1E;border:1px solid #1A1A32;border-radius:10px;padding:10px 12px;margin-bottom:8px">'
        +'<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px">'
          +'<div style="font-size:13px;font-weight:600;color:#EDEDFF;flex:1">'+e.n+'</div>'
          +(e.nota?'<div style="font-size:10px;font-family:\'JetBrains Mono\',monospace;color:#444466;flex-shrink:0;text-align:right">'+e.nota+'</div>':'')
        +'</div>'
        +(e.simple?'<div style="font-size:11px;color:#7070AA;margin-top:4px;line-height:1.5">'+e.simple+'</div>':'')
