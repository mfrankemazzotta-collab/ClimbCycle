...
function r2(l,v){return '<div><div style="font-size:11px;color:var(--text-muted)">'+l+'</div><div style="font-family:\'JetBrains Mono\',monospace;font-size:13px;color:var(--text-primary)">'+v+'</div></div>';}
...
function showDD(date,plan){
    ...
    body+=exs.map(function(e){
      return '<div style="background:var(--bg-card);border:1px solid var(--border-color);border-radius:10px;padding:10px 12px;margin-bottom:8px">'
        +'<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px">'
          +'<div style="font-size:13px;font-weight:600;color:var(--text-primary);flex:1">'+escapeHtml(e.n)+'</div>'
  ...
  var actionsHtml='<div style="margin-top:16px;border-top:1px solid var(--border-color);padding-top:14px">'
    +'<div style="font-size:11px;font-family:\'JetBrains Mono\',monospace;color:var(--text-muted);text-transform:uppercase;letter-spacing:1px;margin-bottom:10px">Cambiar este día</div>'
...
function togglePhDet(id){
  ...
  var open = el.style.display !== 'none';
  el.style.display = open ? 'none' : 'block';
  /* label lives in its own span so the whole pill can be the tap target */
  var lbl = document.getElementById(id+'-lbl');
  if(lbl){
    lbl.textContent = open ? '+ ver' : '- ocultar';
    lbl.style.color = open ? 'var(--text-secondary)' : 'var(--accent-primary-d)';
  }
  if(btn) btn.style.borderColor = open ? 'var(--border-color)' : 'var(--accent-primary)';
}
...
──────────────────────────────────────────────────── */
/* ────────────────────────────────────────────────────
   renderExerciseCard(ex, plan, exCol, eid, extraStyle)
   The "rich" exercise card of the day panel (showDayPanel). Was duplicated
   verbatim in the main-phase and secondary-phase loops; unified here so both
   call sites (and any future one) share a single source of truth.
   `extraStyle` is appended to the card's outer style (secondary phases pass a
   tint + indent). renderProgressionBadge lives in render-home (not loaded by
   the test harness) so it's called through a typeof guard.
──────────────────────────────────────────────────── */
function renderExerciseCard(ex, plan, exCol, eid, extraStyle, opts){
  if(!ex) return '';
  opts = opts || {};
  var tint = function(pct){ return 'color-mix(in srgb, '+exCol+' '+pct+'%, transparent)'; };
  /* El chip lleva una etiqueta CORTA, no la descripción larga de SYS_HUMAN
     (42-77 chars): con flex:none esa descripción se comía todo el ancho y
     aplastaba el nombre hasta partirlo letra por letra. Las claves largas de
     `sys` se abrevian acá para que entren sin truncarse en móvil. */
  var sysChip = (typeof SYS_CHIP !== 'undefined' && SYS_CHIP[ex.sys]) ? SYS_CHIP[ex.sys] : (ex.sys || '');
  var sci = ex.sci || '';
  var guide = (typeof renderExerciseGuide === 'function') ? renderExerciseGuide(ex, eid, exCol) : '';

  /* ① dosis grande + descanso (lo único que se relee entre series) */
  var d = (typeof splitDose === 'function') ? splitDose(ex) : {dose:(ex.nota||''), rest:''};

  /* ② carga concreta en kg, derivada del test del escalador */
  var kgHtml = '';
  var pg = null, grip = '';
  if(plan && plan.week && typeof getWeekProgression === 'function' && typeof getWeekInPhase === 'function'){
    var wi = plan.week - 1, wip = getWeekInPhase(wi), plen = getPhaseLength(wi);
    pg = getWeekProgression(ex.cat, wip, plen);
    if(ex.cat === 'finger_strength' && typeof getGripForWeek === 'function') grip = getGripForWeek(wip, U.level) || '';
    if(pg && pg.load && typeof getCategoryLoad === 'function'){
      var L = getCategoryLoad(ex.cat, pg.load);
      if(L){
        var nota = (typeof staleLoadNote === 'function') ? staleLoadNote(L) : '';
        kgHtml = '<div style="font-family:\'JetBrains Mono\',monospace;font-size:19px;font-weight:700;color:'+(L.stale?'var(--text-secondary)':exCol)+'">&#8776; '+L.kg+' kg</div>'
          +'<div style="font-size:11px;color:var(--text-secondary);margin-top:1px">'+L.pct+'% de tu '+L.label+' ('+L.max+' kg)</div>'
          /* dato viejo: se muestra igual (una referencia con salvedad es más
             útil que ninguna), pero sin la confianza visual de un test fresco */
          +(nota ? '<div style="font-size:11px;color:var(--accent-caution);margin-top:2px">&#9888; '+escapeHtml(nota)+'</div>' : '');
      }
    }
  }

  /* ③ check por ejercicio — zona del pulgar, 44px (solo donde se pide) */
  var check = '';
  if(opts.check){
    check = '<button onclick="'+(opts.onCheck||'')+'" aria-label="'+(opts.done?'Desmarcar':'Marcar')+' '+escapeHtml(ex.n)+'" style="width:44px;height:44px;flex:none;display:flex;align-items:center;justify-content:center;background:none;border:none;cursor:pointer;touch-action:manipulation">'
      +'<span style="width:26px;height:26px;border-radius:50%;display:flex;align-items:center;justify-content:center;'
      +(opts.done ? 'background:'+exCol+';border:2px solid '+exCol+';color:var(--bg-card);font-size:15px;font-weight:700' : 'border:2px solid var(--text-muted)')
      +'">'+(opts.done?'&#x2713;':'')+'</span></button>';
  }

  return '<div class="ex-card" style="border-left-color:'+exCol+';padding-left:'+(opts.check?'4px':'12px')+(opts.done?';opacity:.55':'')+(extraStyle ? ';'+extraStyle : '')+'">'
    +'<div style="display:flex;align-items:flex-start;gap:4px">'
      + check
      +'<div style="flex:1;min-width:0'+(opts.check?';padding-top:3px':'')+'">'
        /* nombre + sistema */
        +'<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px">'
          +'<div style="flex:1 1 auto;min-width:0;overflow-wrap:break-word;font-family:\'Barlow Condensed\',sans-serif;font-size:18px;font-weight:700;line-height:1.15;color:var(--text-primary)">'+escapeHtml(ex.n)+'</div>'
          +(sysChip?'<span style="flex:0 1 auto;max-width:38%;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-family:\'JetBrains Mono\',monospace;font-size:11px;padding:2px 8px;border-radius:99px;background:'+tint(12)+';color:'+exCol+'">'+escapeHtml(sysChip)+'</span>':'')
        +'</div>'
        /* dosis | kg — nivel "de reojo".
           min-width:0 es imprescindible: sin eso las celdas de grilla no encogen
           por debajo de su contenido y el texto se sale de la tarjeta en móvil.
           El cuerpo baja de 19px si la dosis es larga, para que siempre entre. */
        +'<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:8px;align-items:start">'
          +'<div style="min-width:0;overflow-wrap:anywhere'+(kgHtml?';border-right:1px solid var(--divider);padding-right:10px':'')+'">'
            +'<div style="font-family:\'JetBrains Mono\',monospace;font-size:'+(d.dose.length>13?'14px':d.dose.length>9?'16px':'19px')+';font-weight:700;color:var(--text-primary);line-height:1.15">'+escapeHtml(d.dose)+'</div>'
            +(d.rest?'<div style="font-size:11px;color:var(--text-secondary);margin-top:2px">descanso '+d.rest+'</div>':'')
            +(d.detail?'<div style="font-size:11px;color:var(--text-muted);margin-top:2px;line-height:1.35">'+escapeHtml(d.detail)+'</div>':'')
          +'</div>'
          +(kgHtml?'<div style="min-width:0;overflow-wrap:anywhere">'+kgHtml+'</div>':'')
        +'</div>'
        /* progresión — la única pieza tintada */
        +(pg?'<div style="margin-top:8px;border:1px solid '+tint(27)+';background:'+tint(6)+';border-radius:8px;padding:6px 10px">'
          +'<div style="display:flex;justify-content:space-between;gap:8px;align-items:baseline">'
            +'<span style="font-family:\'JetBrains Mono\',monospace;font-size:11px;font-weight:700;letter-spacing:.6px;text-transform:uppercase;color:'+exCol+'">'+escapeHtml(pg.tag||'')+'</span>'
            +(grip?'<span style="font-family:\'JetBrains Mono\',monospace;font-size:11px;color:var(--text-secondary)">'+escapeHtml(grip)+'</span>':'')
          +'</div>'
          +(pg.mod?'<div style="font-size:11.5px;color:var(--text-secondary);line-height:1.45;margin-top:2px">'+escapeHtml(pg.mod)+'</div>':'')
        +'</div>':'')
        /* explicación "para todos" */
        +(ex.simple?'<div style="font-size:12.5px;color:var(--text-secondary);line-height:1.5;margin-top:8px">'+escapeHtml(ex.simple)+'</div>':'')
        + guide
        +(sci?'<div style="margin-top:6px"><button id="btn'+eid+'" onclick="tgSci(\''+eid+'\')" style="background:none;border:none;color:var(--text-secondary);font-size:11px;font-family:\'JetBrains Mono\',monospace;cursor:pointer;padding:0;min-height:44px">+ ver ciencia</button>'
          +'<div id="sci'+eid+'" style="display:none;font-size:11.5px;color:var(--text-muted);margin-top:4px;line-height:1.5;border-top:1px solid var(--border-color);padding-top:6px">'+(typeof autoTerm === 'function' ? autoTerm(sci) : escapeHtml(sci))+'</div></div>':'')
      +'</div>'
    +'</div>'
  +'</div>';
}
