...
function initApp(){
  ...
  loadRec();
  loadLastEx();
  if(typeof loadExDone === 'function') loadExDone();
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  /* Optional crash reporting — no-op unless CC_SENTRY_DSN is configured. */
  ...
  /* Cifrado en reposo: si el usuario tiene vault, hay que DESBLOQUEARLO
     antes de leer nada. Si arrancáramos igual, `loadU()` leería null y la
     app mostraría onboarding en blanco — el usuario creería que perdió
     todo. `ccVaultGate` devuelve false cuando toma el control de la
     pantalla; llama a arrancarApp() cuando termina. Inerte sin el flag. */
  if(typeof ccVaultGate === 'function' && !ccVaultGate(arrancarApp)){
    return;
  }
...
/* Arranque de la app propiamente dicho: de acá para abajo se asume que los
   datos ya son legibles (vault desbloqueado, o sin vault). */
...
function goPage(id){
  ...
  try{if(id==='hoy'&&typeof renderHoy==='function')renderHoy();}catch(e){if(typeof logError==='function')logError(e,'renderHoy');}
