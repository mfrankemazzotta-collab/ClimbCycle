...
function openQA(){
  ...
  /* "I trained today" — catch-up / off-plan training on a rest day. On a
     planned training day the explicit "marcar hecha" above already covers it. */
  if(!tp || tp.block === 'rest'){
    actions.push({
      icon:'&#x1F3CB;',
      title:'Entrené hoy igual',
      sub:'Fui al gym aunque hoy no tocaba — registralo y el plan lo cuenta',
      col:'var(--accent-deload)',
      onclick:'closeQA();trainedToday()'
    });
  }
  ...
  if(hero){
    if(tp && tp.block !== 'rest'){
      ...
      hero.innerHTML = '<div style="position:relative;background:linear-gradient(135deg,'+bt.col+'18,'+bt.col+'08);border:1px solid '+bt.col+'33;border-radius:18px;padding:20px;margin-bottom:18px;overflow:hidden">'
        ...
        +'<div style="position:relative;z-index:1">'
          +'<div style="display:flex;align-items:center;gap:8px;margin-bottom:10px">'
            +'<span style="font-family:\'JetBrains Mono\',monospace;font-size:11px;color:'+stypeCol+';background:'+stypeCol+'22;border:1px solid '+stypeCol+'44;padding:3px 9px;border-radius:99px;font-weight:700;letter-spacing:0.5px">'+(sessionType?sessionType.toUpperCase():'HOY')+'</span>'
            +(logSt==='done'?'<span style="font-family:\'JetBrains Mono\',monospace;font-size:11px;color:#00E5A0;background:#00E5A022;padding:3px 9px;border-radius:99px;font-weight:700">COMPLETADA</span>':'')
            +(logSt==='fail'?'<span style="font-family:\'JetBrains Mono\',monospace;font-size:11px;color:#FF4D6A;background:#FF4D6A22;padding:3px 9px;border-radius:99px;font-weight:700">NO HECHA</span>':'')
          +'</div>'
      ...
      hero.innerHTML = '<div style="background:linear-gradient(135deg,#9B6EFF18,#9B6EFF08);border:1px solid #9B6EFF33;border-radius:18px;padding:20px;margin-bottom:18px">'
        +'<span style="font-family:\'JetBrains Mono\',monospace;font-size:11px;color:#9B6EFF;background:#9B6EFF22;border:1px solid #9B6EFF44;padding:3px 9px;border-radius:99px;font-weight:700">ROCA EXTERIOR</span>'
      ...
      hero.innerHTML = '<div style="background:var(--bg-card);border:1px solid var(--border-color);border-radius:18px;padding:24px;margin-bottom:18px;text-align:center">'
        +'<div style="font-family:\'JetBrains Mono\',monospace;font-size:11px;color:var(--text-muted);letter-spacing:1.5px;text-transform:uppercase;margin-bottom:8px">Hoy</div>'
...
function renderProgressionBadge(ex, plan, exCol){
  ...
  if(typeof getCategoryLoad === 'function' && pg.load){
    ...
    if(L){
      var notaL = (typeof staleLoadNote === 'function') ? staleLoadNote(L) : '';
      loadHtml = '<div style="margin-top:5px;font-family:\'JetBrains Mono\',monospace;font-size:12px;font-weight:700;color:' + (L.stale ? 'var(--text-secondary)' : exCol) + '">'
        + '&#x2248; ' + L.kg + ' kg'
        + '<span style="font-size:11px;font-weight:400;color:var(--text-muted)"> · ' + L.pct + '% de tu ' + L.label + ' (' + L.max + ' kg)'
        + (L.adjusted ? ' · ajustado a tu test' : '') + '</span>'
        + (notaL ? '<div style="font-size:11px;font-weight:400;color:var(--accent-caution);margin-top:2px">&#9888; ' + escapeHtml(notaL) + '</div>' : '')
      + '</div>';
    }
  }
...
function showDayPanel(date,plan,key){
  ...
  if(!plan||plan.block==='rest'){
    ...
    var rockBtn = rockNote
      ...
      : '<button onclick="markRockDay(\''+key+'\');" style="margin-top:8px;width:100%;padding:9px;background:none;border:1.5px solid #9B6EFF55;border-radius:10px;color:#9B6EFF;font-size:12px;font-family:\'JetBrains Mono\',monospace;cursor:pointer">+ Marcar como roca exterior</button>'
        +'<div style="font-size:11px;color:var(--text-muted);margin-top:4px;text-align:center">El plan se ajusta automaticamente</div>';
    var overrideBtn = gapNote
      ? '<button onclick="forceSession(\''+key+'\');" style="margin-top:6px;width:100%;padding:9px;background:none;border:1.5px solid #FFB800;border-radius:10px;color:#FFB800;font-size:12px;font-family:\'JetBrains Mono\',monospace;cursor:pointer">Forzar sesión de todas formas</button>'
        +'<div style="font-size:11px;color:var(--text-muted);margin-top:4px;text-align:center">No recomendado</div>'
      : '';
  ...
  if(bt.sessionType && bt.sessionType!=='Rest' && bt.sessionType!=='Test'){
    ...
    stypeBadge = '<div style="display:flex;align-items:center;gap:6px;margin-bottom:8px;flex-wrap:wrap">'
      +'<span style="font-size:11px;font-family:\'JetBrains Mono\',monospace;padding:3px 10px;border-radius:99px;'
        +'border:1px solid '+stypeCol+'44;background:'+stypeCol+'15;color:'+stypeCol+';font-weight:700">'
        +bt.sessionType.toUpperCase()+'</span>'
      +(bt.dial?'<span style="font-size:11px;color:var(--text-muted);font-family:\'JetBrains Mono\',monospace">Dial: '+bt.dial+'</span>':'')
      +(bt.faderRange?'<span style="font-size:11px;color:var(--text-muted);font-family:\'JetBrains Mono\',monospace">Fader: '+bt.faderRange+'/10</span>':'')
    +'</div>'
    +(bt.faderDesc?'<div style="font-size:11px;color:var(--text-secondary);margin-bottom:8px;line-height:1.5">'+bt.faderDesc+'</div>':'');
  }
  ...
  if(bt.goalFocus && U.goal && bt.goalFocus[U.goal]){
    goalFocusHtml = '<div style="background:var(--bg-card-alt);border-left:2px solid '+bt.col+'66;border-radius:0 6px 6px 0;padding:6px 10px;margin-bottom:10px">'
      +'<div style="font-size:11px;font-family:\'JetBrains Mono\',monospace;color:var(--text-muted);text-transform:uppercase;letter-spacing:1px;margin-bottom:2px">Enfoque SAID para tu objetivo</div>'
  ...
  var progBar = '<div style="margin-bottom:12px">'
    +'<div style="display:flex;justify-content:space-between;margin-bottom:4px">'
      +'<span style="font-size:11px;color:var(--text-secondary);font-family:\'JetBrains Mono\',monospace">Semana '+plan.week+'</span>'
      +'<span style="font-size:11px;font-family:\'JetBrains Mono\',monospace;color:'+progCol+'">'+wkComp.done+'/'+wkComp.total+' ('+wkComp.pct+'%)</span>'
    +'</div>'
    +'<div class="mtr"><div class="mf" style="width:'+wkComp.pct+'%;background:'+progCol+'"></div></div>'
    +(wkComp.pct<70&&wkComp.total>0?'<div style="font-size:11px;color:#FFB800;margin-top:3px">Completa el 70% para avanzar de semana</div>':'')
    +'</div>';
  ...
  if(state==='available'||state==='missed'){
    actHtml = '<div style="display:flex;gap:6px;margin-top:10px">'
      ...
      +'<div style="font-size:11px;color:var(--text-muted);margin-top:3px;text-align:center">El plan se ajusta automaticamente</div>';
  ...
  if(phases.length > 0){
    ...
    /* Whole pill is tappable (not just the button): on narrow phones the flex row
       could overflow and leave the button's tap area clipped off-screen. */
    exHtml += '<button id="'+collapseId+'-btn" onclick="togglePhDet(\''+collapseId+'\')" style="width:100%;text-align:left;margin-top:14px;margin-bottom:14px;padding:10px 12px;background:var(--bg-card);border:1px solid var(--border-color);border-radius:10px;display:flex;justify-content:space-between;align-items:center;gap:10px;cursor:pointer;touch-action:manipulation">'
      +'<span style="min-width:0">'
        +'<span style="display:block;font-family:\'Barlow Condensed\',sans-serif;font-size:14px;font-weight:700;color:var(--text-primary);line-height:1.1">Sesión de '+totalMin+' min</span>'
        +'<span style="display:block;font-size:11px;color:var(--text-muted);margin-top:2px">'+phases.length+' fases &middot; adaptada a tu perfil</span>'
      +'</span>'
      +'<span id="'+collapseId+'-lbl" style="flex-shrink:0;font-size:11px;font-family:\'JetBrains Mono\',monospace;color:var(--text-secondary);font-weight:700">+ ver</span>'
    +'</button>';
    ...
    if(mainPh){
      ...
      exHtml += '<div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;padding-bottom:6px;border-bottom:1px solid var(--divider)">'
        ...
        +'<div style="font-family:\'JetBrains Mono\',monospace;font-size:11px;color:var(--text-primary);text-transform:uppercase;letter-spacing:1.2px;font-weight:700">Entrenamiento principal &middot; '+mainPh.minutes+' min</div>'
      +'</div>';
      /* Main exercises */
      if(mainExs.length > 0){
        mainExs.forEach(function(ex, ei){
          ...
          exHtml += renderExerciseCard(ex, plan, exCol, eid, '');
        });
    ...
    exHtml += '<div style="font-family:\'JetBrains Mono\',monospace;font-size:11px;color:var(--text-muted);text-transform:uppercase;letter-spacing:1.2px;margin-bottom:10px;font-weight:700">Estructura completa de la sesión</div>';
    ...
    phases.forEach(function(ph, pi){
      ...
      exHtml += '<div style="margin-top:14px;margin-bottom:8px;display:flex;align-items:center;gap:10px;padding:10px 12px;background:'+ph.col+'10;border:1px solid '+ph.col+'2A;border-radius:10px">'
        ...
        +'<div style="flex:1">'
          ...
          +'<div style="font-size:11px;color:var(--text-secondary);line-height:1.3">'+ph.desc+'</div>'
        +'</div>'
      ...
      if(phEx.length > 0){
        phEx.forEach(function(ex, ei){
          ...
          exHtml += renderExerciseCard(ex, plan, exCol, eid, 'background:'+exCol+'08;margin-left:10px');
        });
        ...
        phTests.forEach(function(t){
          exHtml += '<div class="ex-card" style="border-left-color:#FFB800;background:#FFB80008;margin-left:10px">'
            ...
            +'</div>'
            +'<div class="ex-det" style="font-size:11px;color:var(--text-secondary);line-height:1.5;margin-top:4px">'+t.mide+'</div>'
            +'<div style="font-family:\'JetBrains Mono\',monospace;font-size:11px;color:var(--accent-primary-d);background:var(--accent-primary-bg);border-radius:5px;padding:6px 10px;margin-top:6px;line-height:1.6">'+t.how+'</div>'
    ...
    if(warmupExs.length > 0){
      exHtml += '<div style="margin-top:14px;margin-bottom:8px;display:flex;align-items:center;gap:8px"><div style="font-family:\'JetBrains Mono\',monospace;font-size:11px;color:#FFB800;text-transform:uppercase;letter-spacing:1.5px;font-weight:700">Calentamiento</div></div>';
      warmupExs.forEach(function(ex){
        exHtml += '<div class="ex-card" style="border-left-color:#FFB800"><div class="ex-name" style="color:#FFB800">'+escapeHtml(ex.n)+'</div></div>';
      });
    }
    exs.forEach(function(ex){
      exHtml += '<div class="ex-card" style="border-left-color:'+(ex.col||bt.col)+'"><div class="ex-name">'+escapeHtml(ex.n)+'</div></div>';
    });
  }
  ...
  p.innerHTML = '<div class="daypanel" style="border-left:3px solid '+bt.col+'">'
    +'<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:4px">'
      ...
      +(isT?'<span style="font-family:\'JetBrains Mono\',monospace;font-size:11px;color:var(--accent-primary-d);background:var(--accent-primary-bg);padding:3px 8px;border-radius:99px">HOY</span>':'')
    +'</div>'
    + stypeBadge
...
/* "I trained today" — for the disorganized climber. Decides (via the pure
   resolveTrainedToday) whether to mark today, pull a pending session onto today,
   or log a bonus, then applies it with the existing session primitives. */
function trainedToday(){
  if(!U.startDate) return;
  var todayKey = TODAY.toDateString();

  /* Plan hasn't started yet (start date is in the future)? "I trained today"
     clearly means "I'm starting now" — offer to move the start to today so the
     session counts. No progress is lost (the plan never began). */
  var start0 = new Date(U.startDate); start0.setHours(0,0,0,0);
  if(TODAY < start0){
    var startTxt = start0.toLocaleDateString('es-ES', {day:'numeric', month:'long'});
    var startPlanToday = function(){
      U.startDate = new Date(TODAY.getFullYear(), TODAY.getMonth(), TODAY.getDate());
      if(typeof generatePlan === 'function') generatePlan();
      if(typeof saveU === 'function') saveU();
      sessionLog[todayKey] = 'done';
      logSessionDone(todayKey);   /* que cuente como carga (ACWR) */
      if(typeof commit === 'function'){ commit('plan'); commit('session'); }
      if(typeof bigDate !== 'undefined') bigDate = new Date(U.startDate);
      hcSel = new Date(TODAY);
      if(typeof renderWidgets === 'function') renderWidgets();
      if(typeof showToast === 'function') showToast('¡Plan empezado hoy! Sesión registrada.', 'var(--accent-deload)');
    };
    if(typeof confirmDialog === 'function'){
      confirmDialog({ title:'Tu plan arranca el ' + startTxt, message:'Todavía no empezó. ¿Lo empezamos hoy así cuenta la sesión que hiciste?', confirm:'Empezar hoy', cancel:'Todavía no' })
        .then(function(ok){ if(ok) startPlanToday(); });
    } else {
      startPlanToday();
    }
    return;
  ...
  var curWk = getCurrentWeekIndex();
  var weekDays = [];
  for(var di = 0; di < 7; di++){
    var date = new Date(U.startDate); date.setDate(date.getDate() + curWk*7 + di);
    var key = date.toDateString();
    var plan = planMap[key];
    var isTraining = !!(plan && plan.block !== 'rest' && plan.block !== 'test' && !plan.outdoor);
    weekDays.push({ date:key, isTraining:isTraining, done: getSessionState(key, plan) === 'completed', isToday: key === todayKey });
  }
  var r = resolveTrainedToday(weekDays);
  if(r.action === 'alreadyDone'){
    if(typeof showToast === 'function') showToast('Ya registraste tu sesión de hoy','var(--text-secondary)');
    return;
  }
  if(r.action === 'markToday'){ markSess(todayKey, 'done'); return; }
  if(r.action === 'moveHere'){
    var fp = planMap[r.from];
    planMap[todayKey] = { block: fp ? fp.block : 'endurance', week: (fp && fp.week) || (curWk + 1), moved:true };
    if(fp) fp.movedTo = todayKey;
    sessionLog[r.from] = 'moved';
    sessionLog[todayKey] = 'done';
    logSessionDone(todayKey);   /* que cuente como carga (ACWR) */
    commit('plan'); commit('session');
    if(typeof showToast === 'function') showToast('¡Listo! Tu sesión de hoy quedó registrada','var(--accent-deload)');
  } else {  /* bonus */
    setDayTraining(todayKey);
    sessionLog[todayKey] = 'done';
    logSessionDone(todayKey);   /* que cuente como carga (ACWR) */
    commit('plan'); commit('session');
    if(typeof showToast === 'function') showToast('Sesión extra registrada hoy','var(--accent-deload)');
  }
  if(hcSel && hcSel.toDateString() === todayKey) showDayPanel(hcSel, planMap[todayKey], todayKey);
}
function markSess(dstr,status){
  sessionLog[dstr]=status;
  if(status === 'done') logSessionDone(dstr);
  else {
    /* No realizada: ni carga, ni ejercicios tildados. */
    if(typeof unlogAutoSession === 'function') unlogAutoSession(dstr);
    if(typeof clearExDone === 'function' && clearExDone(dstr)) commit('exdone', false);
  }
  commit('session');
...
function undoSess(dstr){
  delete sessionLog[dstr];
  if(typeof unlogAutoSession === 'function') unlogAutoSession(dstr);
  /* Deshacer devuelve el día a cero: si no, la sesión queda "abierta" pero con
     todos los ejercicios hechos, y para volver a cerrarla hay que desmarcar y
     remarcar uno. */
  if(typeof clearExDone === 'function' && clearExDone(dstr)) commit('exdone', false);
  commit('session');
  if(hcSel&&hcSel.toDateString()===dstr)showDayPanel(hcSel,planMap[dstr],dstr);
...
function openMvM(dstr,btype){
  ...
  for(var i=1;i<=7;i++){
    ...
    btn.innerHTML='<span>'+DLG[date.getDay()]+' '+date.getDate()+'/'+('0'+(date.getMonth()+1)).slice(-2)+'</span>'
      +(busy?'<span style="font-size:11px;color:#FFB800">sesión existente</span>':'<span style="font-size:11px;color:var(--text-muted)">libre</span>');
...
function doMv(fromK,btype,toK,toDate,busy){
  ...
  sessionLog[fromK]='moved';commit('session');
