...
function buildExTab(){
  ...
  if(todayPlan && todayPlan.block!=='rest' && todayPlan.block!=='test'){
    ...
    todayHtml='<div style="margin-bottom:22px">'
      +'<div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;padding-bottom:10px;border-bottom:1px solid var(--border-color)">'
        ...
        +'<span style="font-size:11px;font-family:\'JetBrains Mono\',monospace;color:'+todayBt.col+';background:'+todayBt.col+'18;padding:3px 9px;border-radius:99px;letter-spacing:0.5px;font-weight:700;margin-left:auto">'+todayBt.label.toUpperCase()+'</span>'
      +'</div>';
  ...
  if(weekKeys.length>0){
    weekHtml='<div style="margin-bottom:22px">'
      +'<div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;padding-bottom:10px;border-bottom:1px solid var(--border-color)">'
        ...
        +'<span style="font-size:11px;color:var(--text-secondary);font-family:\'JetBrains Mono\',monospace;background:var(--bg-card-alt);padding:3px 9px;border-radius:99px;margin-left:auto">'+weekKeys.length+' ejercicios</span>'
      +'</div>';
  ...
  filterBar+='<button onclick="exFilter(\'all\')" '
    ...
    +'font-family:\'JetBrains Mono\',monospace;font-size:11px;cursor:pointer;touch-action:manipulation">Todos</button>';
  Object.keys(CAT_META).forEach(function(cat){
    ...
    filterBar+='<button onclick="exFilter(\''+cat+'\')" '
      ...
      +'font-family:\'JetBrains Mono\',monospace;font-size:11px;cursor:pointer;touch-action:manipulation;white-space:nowrap">'
    ...
    Object.keys(CAT_META).forEach(function(cat){
      ...
      catalogHtml+='<div style="margin-bottom:14px">'
        +'<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;padding:6px 0;border-bottom:1px solid '+meta.col+'33">'
          ...
          +'<span style="font-size:11px;color:var(--text-muted)">'+byCat[cat].length+'</span>'
        +'</div>'
        +'<div style="font-size:11px;color:var(--text-muted);margin-bottom:8px;padding-left:22px">'+meta.desc+'</div>';
...
function renderExCard(ex,col,isWarmup,dayLabel){
  ...
  var html='<div style="background:'+bgCol+';border:1px solid var(--border-color);border-radius:10px;padding:11px;margin-bottom:6px;border-left:3px solid '+borderCol+'">'
    +'<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:3px;gap:8px">'
      +'<div style="font-size:13px;font-weight:600;color:var(--text-primary);flex:1">'+escapeHtml(ex.n)+'</div>'
      +'<div style="display:flex;gap:4px;align-items:center;flex-shrink:0">';
  if(isWarmup) html+='<span style="font-size:11px;color:#FFB800;background:#FFB80018;padding:2px 6px;border-radius:4px;font-family:\'JetBrains Mono\',monospace">warm-up</span>';
  if(dayLabel) html+='<span style="font-size:11px;color:'+col+';background:'+col+'18;padding:2px 6px;border-radius:4px;font-family:\'JetBrains Mono\',monospace">'+dayLabel.slice(0,3)+'</span>';
  html+='</div></div>';
  if(humanSys) html+='<div style="font-size:11px;color:'+col+';margin-bottom:4px">'+humanSys+'</div>';
  if(nota) html+='<div style="font-family:\'JetBrains Mono\',monospace;font-size:11px;color:var(--accent-primary-d);background:var(--accent-primary-bg);border-radius:5px;padding:4px 8px;margin-bottom:6px">'+nota+'</div>';
  ...
  if(sci) html+='<div style="font-size:11px;color:var(--text-muted);margin-top:6px;line-height:1.5;border-top:1px solid var(--border-color);padding-top:6px">'+(typeof autoTerm==='function'?autoTerm(sci):sci)+'</div>';
  html+='</div>';
  return html;
}
...
function buildHBTab(){
  ...
  HBP.filter(function(p){return p.hb;}).forEach(function(p){
    h+='<div class="proto" style="border-left:3px solid '+p.col+'"><div style="display:flex;justify-content:space-between;margin-bottom:6px"><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:15px;font-weight:700;color:var(--text-primary)">'+p.t+'</div><span style="font-size:11px;font-family:\'JetBrains Mono\',monospace;color:'+p.col+'">'+p.ph+'</span></div><div class="proto-nota">'+p.nota+'</div><div style="font-size:12px;color:var(--text-secondary);line-height:1.6;margin-bottom:8px">'+p.desc+'</div><div style="margin-bottom:8px">';
