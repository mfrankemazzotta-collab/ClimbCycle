...
function renderTestChart(model, col){
  ...
  return '<div class="tdb-chart" style="margin-top:10px">'
    + '<div style="display:flex;justify-content:space-between;align-items:center;margin:0 2px 2px">'
      + '<span style="font-size:11px;font-family:\'JetBrains Mono\',monospace;color:var(--text-muted)">Progreso · '+model.count+' registros</span>'
      + '<span style="font-size:11px;font-family:\'JetBrains Mono\',monospace;font-weight:700;color:'+tcol+'">'+arrow+' '+(tr>0?'+':'')+tr+'% vs 1er test</span>'
    + '</div>'
  ...
  if(r){
    ...
    barHTML=
      ...
      +'<div style="text-align:right;margin-top:4px">'
        +'<span style="font-size:11px;font-family:\'JetBrains Mono\',monospace;color:'+(pctVsMid>=0?'var(--accent-deload)':'var(--accent-caution)')+'">'
...
/* Pure view-model for the "next test recommendation" banner: maps a test's
   status (from nextTestRecommendation().status) to its color + message.
   Extracted from buildTsTab so the branching logic is unit-testable. */
function tsRecView(st){
  return {
    color: st.neverDone ? 'var(--accent-primary)'
         : st.overdue ? 'var(--accent-warning)'
         : st.daysUntil <= 7 ? 'var(--accent-caution)'
         : 'var(--accent-deload)',
    msg: st.neverDone ? 'Nunca hecho - hazlo al comienzo de tu próximo día fresco'
       : st.overdue ? 'Hace ' + st.daysSince + ' días - ya toca repetirlo'
...
function buildTsTab(){
  ...
  if(rec){
    var st=rec.status;
    var _rv=tsRecView(st);
    var recCol=_rv.color;
    var recMsg=_rv.msg;
    recHtml='<div style="background:'+recCol+'18;border:1px solid '+recCol+'44;border-radius:12px;padding:12px;margin-bottom:14px">'
      +'<div style="font-size:11px;font-family:\'JetBrains Mono\',monospace;color:'+recCol+';text-transform:uppercase;letter-spacing:1px;margin-bottom:6px">Próximo test recomendado</div>'
        ...
        +'<div style="text-align:right;flex-shrink:0">'
          +(st.neverDone
            ...
            :'<div style="font-family:\'JetBrains Mono\',monospace;font-size:22px;font-weight:700;color:'+recCol+'">'+st.pct+'%</div>'
              +'<div style="font-size:11px;color:var(--text-muted)">del intervalo</div>')
        +'</div>'
      +'</div>'
  ...
  var infoHtml='<div style="background:var(--bg-card-alt);border:1px solid var(--border-color);border-radius:10px;padding:10px 12px;margin-bottom:14px;font-size:11px;color:var(--text-muted);line-height:1.6">'
    +'<span style="color:var(--accent-primary-d);font-family:\'JetBrains Mono\',monospace;font-size:11px;text-transform:uppercase;letter-spacing:1px">Tests vs Entrenamientos</span><br>'
  ...
  TESTS.forEach(function(t,ti){
    ...
    h+='<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px;cursor:pointer" onclick="tgTR('+ti+')">'
      +'<div style="flex:1">'
        ...
        +'<div style="font-size:11px;color:var(--text-muted);margin-top:1px">'+t.mide+'</div>'
        +'<div style="font-size:11px;color:'+stCol+';margin-top:3px">'+stTxt+'</div>'
        +stBar
        +(st.lastDate?'<div style="font-size:11px;color:var(--text-muted);margin-top:2px">Último: '+st.lastDate+'</div>':'')
      +'</div>'
      +'<div style="display:flex;flex-direction:column;align-items:flex-end;gap:4px;flex-shrink:0;margin-left:8px">'
        +'<span style="font-size:11px;font-family:\'JetBrains Mono\',monospace;padding:3px 8px;border-radius:99px;border:1px solid;color:'+t.col+';border-color:'+t.col+'">'+t.diff+'</span>'
        +'<span style="font-size:11px;color:var(--text-muted)">'+t.freq+'</span>'
      +'</div>'
    +'</div>';
    ...
    h+='<div id="trbdy'+ti+'" style="display:none;margin-top:10px;padding:10px;background:var(--bg-card-alt);border-radius:8px">'
      +'<div style="font-size:11px;color:var(--accent-primary-d);font-family:\'JetBrains Mono\',monospace;margin-bottom:8px;text-transform:uppercase;letter-spacing:1px">Como hacerlo:</div>'
  ...
  if(cal.hasAny){
    var calRows = cal.items.map(function(it){
      ...
      return '<div style="display:flex;justify-content:space-between;align-items:baseline;gap:8px;padding:4px 0;border-top:1px solid var(--border-color)">'
        ...
        +'<span style="font-family:\'JetBrains Mono\',monospace;font-size:12px;color:var(--accent-primary-d);font-weight:700;text-align:right">'+it.max+' kg'
          +(adjTxt?'<span style="font-size:11px;color:var(--text-muted);font-weight:400">'+adjTxt+'</span>':'')+'</span>'
      +'</div>';
    }).join('');
    h+='<div style="margin-top:8px;background:#CCFF0012;border:1px solid #CCFF0033;border-radius:10px;padding:12px">'
      +'<div style="font-size:11px;color:var(--accent-primary-d);font-family:\'JetBrains Mono\',monospace;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px">Tu plan calibrado a tus tests</div>'
      +calRows
    ...
    h+='<div style="margin-top:8px;background:var(--bg-card-alt);border:1px dashed var(--border-color);border-radius:10px;padding:12px">'
      +'<div style="font-size:11px;color:var(--text-muted);font-family:\'JetBrains Mono\',monospace;text-transform:uppercase;letter-spacing:1px;margin-bottom:4px">Aún sin calibrar</div>'
  ...
  h+='<div style="margin-top:8px;background:#CCFF0012;border:1px solid #CCFF0033;border-radius:10px;padding:12px">'
    +'<div style="font-size:11px;color:var(--accent-primary-d);font-family:\'JetBrains Mono\',monospace;text-transform:uppercase;letter-spacing:1px;margin-bottom:4px">Cuando repetir</div>'
