...
/* Los intentos también son carga: un día de proyecto es la escalada más
   intensa que hace el usuario, y si no llega a `cc_logs` el ACWR queda ciego
   justo en el escenario más riesgoso. `syncProjectLoad` agrupa por día y
   respeta cualquier sesión ya registrada (ver recovery.js). */
function _projSyncLoad(list){
  if(typeof syncProjectLoad !== 'function') return;
  try { syncProjectLoad(list); }
  catch(e){ if(typeof logError === 'function') logError(e, 'syncProjectLoad'); }
}

function projAttempt(id){
  var list = logAttemptIn(loadProjects(), id, {}, _projDateTs(id));
  saveProjects(list);
  _projSyncLoad(list);
  if(typeof renderProjects === 'function') renderProjects();
}
function projSend(id){
  var list = logAttemptIn(loadProjects(), id, { sent:true }, _projDateTs(id));
  saveProjects(list);
  _projSyncLoad(list);
      ...
      h += '<div style="background:var(--bg-card-alt);border:1px solid var(--border-color);border-left:3px solid ' + pr.col + ';border-radius:10px;padding:10px 12px;margin-bottom:8px">'
        + '<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px">'
          + '<div style="flex:1;min-width:0">'
            ...
            + '<div style="font-size:11px;color:var(--text-muted);margin-top:1px">' + typeLbl + (p.grade ? (' · ' + esc(p.grade)) : '') + ' · ' + lastTxt + '</div>'
          + '</div>'
          + '<span style="flex-shrink:0;font-size:11px;font-family:\'JetBrains Mono\',monospace;color:' + pr.col + ';background:' + pr.col + '18;border:1px solid ' + pr.col + '44;padding:2px 8px;border-radius:99px">' + pr.label + '</span>'
        + '</div>'
        + '<div class="mtr" style="margin-top:8px"><div class="mf" style="width:' + pr.pct + '%;background:' + pr.col + '"></div></div>'
        + (p.status !== 'sent'
            ? '<div style="display:flex;align-items:center;gap:6px;margin-top:8px">'
              + '<span style="font-size:11px;color:var(--text-muted);white-space:nowrap">Fecha del intento</span>'
