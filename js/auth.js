...
/* ── KEY PREFIX ──
   The per-user localStorage prefixing now lives in storage.js (single owner).
   Auth just tells it who the active user is; the lookup stays dynamic, so it
   always reflects the latest login/logout — same behaviour as the old override. */
if(typeof setStorageUserProvider === 'function') setStorageUserProvider(getCurrentUser);
