...
var TESTS=[
  ...
  },
  {
    ...
    how:'1) Calienta 15-20 min. 2) Determina tu fuerza maxima en regleta de 20mm (Max Hang previo). 3) Calcula 60% de ese maximo. 4) Realiza ciclos de 7s colgado / 3s descanso a esa carga. 5) Continua hasta que NO puedas completar un hang de 7 segundos limpio. 6) Anota los minutos totales. Protocolo de Giles et al. 2019 - rango tipico: 3-7 min principiantes, 5-9 min avanzados.',
...
var BSCI={
  strength:'FASE GO HARD (Dial: Intensivo | Fader: 8-10). '
    ...
    +'Bompa: la fuerza maxima (MxS) requiere 6+ semanas de carga progresiva con buffer descendente. '
    ...
    +'Prioriza UN sistema por sesion: fuerza maxima de dedos (Max Hangs 10s/20mm half-crimp segun Feehally) + traccion (3-5RM pull-ups). '
    ...
    +'Feehally: "tendons don\'t like surprises" - los tendones digitales adaptan 3x mas lento que el musculo.',
  ...
  power:'FASE GO HARD (Dial: Intensivo | Fader: 9-10). '
    ...
    +'Sistema anaerobico alactico (ATP) - 1 a 5 movimientos maximos (Barrows cap.2.5). '
    +'Bompa: esta es la fase de "Conversion" - la fuerza acumulada se convierte en cualidad sport-especifica (potencia explosiva). '
    +'Tkacz: "es neurologica - ensena al SNC a reclutar rapido". Requiere frescura neuromuscular absoluta: minimo 48h entre sesiones. '
  ...
  deload:'FASE EXPLORE (Dial: Extensivo suave | Fader: 2-4). '
    ...
    +'Tkacz: "estudios en medicina deportiva muestran que 5 semanas es el limite que el cuerpo soporta antes de que el progreso decaiga y el riesgo de lesion aumente exponencialmente". '
    +'La supercompensacion - el momento donde el cuerpo realmente mejora - ocurre AQUI, no entrenando. '
    ...
    +'Los tendones adaptan mas lento que el musculo: sin deload, el plan es precursor de lesiones cronicas.',

  test:'Punto A - Evaluacion inicial (Bompa cap.10 + Lattice + Feehally Beastmaking cap.34 Fingerboarding Progress). '
    ...
    +'Feehally: "neuromuscular adaptations come fast at first, then strength gains slow down" - los tests detectan en que fase de adaptacion estas. '
    +'Bompa: los tests al inicio y al fin de cada macrociclo permiten calibrar la carga del proximo ciclo.',
  ...
  rest:'El descanso es donde sucede la adaptacion. El SNC tarda significativamente mas en recuperarse que la musculatura periferica. '
    ...
    +'Horst: sin respeto a los tiempos de recuperacion, el plan es precursor de lesiones cronicas. '
    +'Bompa cap.4 (Fatigue and Recovery): la recuperacion incompleta lleva a "overreaching" funcional, y eventualmente al sobreentrenamiento. '
    +'Feehally (Beastmaking cap.34): "consistency over intensity" - 1 sesion semanal sostenida vale mas que rachas intensas seguidas de parones.'
};
...
/* Human-readable system names with energy-system terminology (Feehally / Beastmaking ch.23-24)
   ...
   - Anaerobic alactic: ATP, 1-5 movimientos maximos */
var SYS_HUMAN={
  'An Cap':       'Anaerobico lactico  -  aguantar 5 a 20 movimientos intensos sin perder fuerza',
  'An Pow':       'Anaerobico alactico  -  explosividad maxima 1 a 5 movimientos',
  'Aero Pow':     'Resistencia con intensidad  -  rutas largas a 60-80% sin bombeo terminal',
  'Aero Cap':     'Base aerobica  -  recuperarse entre movimientos y entre intentos',
  'Fuerza max':   'Fuerza maxima de dedos  -  reclutamiento neural + adaptacion tendinosa',
  'Fuerza traccion':'Fuerza de brazos y espalda para jalar en escalada',
  'Fuerza isometrica':'Sostener posiciones de brazos doblados (lock-offs)',
  'Fuerza max unilateral':'Fuerza de un solo brazo  -  nivel avanzado',
  'Recuperacion activa':'Movimiento suave para acelerar la recuperacion',
  ...
  'Skill development':'Refinar tecnica de movimiento y eficiencia',
...
/* =====================================================
   ...
   - Bompa & Buzzichelli (Periodization Training for Sports, 2015): framework
     ...
     antes de cualquier trabajo de fuerza maxima - prepara tendones y ligamentos.
   - Horst (Training for Climbing, 2016): el macrociclo 4-3-2-1 aplicado a escalada
     organiza el ciclo como 4 sem base aerobica > 3 sem fuerza maxima >
   ...
   - Feehally (Beastmaking): tendones digitales adaptan 3x mas lento que
     musculos. Los principiantes deben evitar la fase de potencia (campus,
     dinamicos al limite) hasta tener base de fuerza.
...
var LEVEL_PROFILES = {
  beginner: {
    /* Principiantes: NO power phase. Bompa cap.11 (Anatomical Adaptation):
       ...
       en muro (volumen tecnico). Sin campus, sin max hangs intensos. */
    phaseSeq:    {
  ...
  advanced: {
    /* Avanzados: pueden empezar con fuerza (frescura neuromuscular maxima).
    ...
    progressNote: 'Progresion semanal con metricas exactas (kg, TTF, liston). Bompa: buffer descendente sesion a sesion.'
  },
  elite: {
    ...
    exPerSession: 4,
    hangboardOk:  true,
    intensityNote: 'Fader 9-10. Autorregulacion por HRV y TTF. Maxima especificidad (Bompa: Conversion to Specific Strength).',
    progressNote: 'Progresion por ciclos de 3 semanas con validacion de tests. Tests al inicio y fin del macrociclo (Bompa cap.10).'
  }
};
