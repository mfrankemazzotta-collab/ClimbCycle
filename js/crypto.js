...
/* ── Claves de datos (DEK) ────────────────────────────
   El vault no cifra con la clave derivada de la contraseña, sino con una
   clave de datos aleatoria que a su vez se guarda envuelta. Eso permite dos
   cosas que con derivación directa serían imposibles: tener una clave de
   recuperación equivalente, y cambiar la contraseña sin recifrar los datos
   (sólo se re-envuelve la DEK). Es el mismo esquema de 1Password/Bitwarden. */

/* Genera una clave AES-GCM aleatoria, devuelta como hex para poder envolverla. */
function ccNewDataKeyHex(){ return ccRandomHex(32); }

/* hex → CryptoKey utilizable para cifrar/descifrar.
   `extractable` queda en true porque cambiar la contraseña exige re-envolver
   la DEK, y para eso hay que poder exportarla. No empeora la superficie: con
   el vault abierto los datos descifrados ya están en memoria de todos modos.
   Vive en un CryptoKey y no en un string JS suelto, que es lo preferible. */
function ccImportDataKey(hex){
  return _ccSubtle().importKey('raw', _ccHexToBytes(hex), { name:'AES-GCM', length:256 },
                               true, ['encrypt','decrypt']);
}
/* CryptoKey → hex (para re-envolver la DEK con otra contraseña). */
function ccExportDataKeyHex(key){
  return _ccSubtle().exportKey('raw', key).then(function(raw){
    return _ccBytesToHex(new Uint8Array(raw));
  });
}
