...
function expect(actual){
  return {
    ...
    notToBe(exp){
      if(actual === exp) throw new Error('expected NOT to be ' + fmt(exp));
    },
    toEqual(exp){
    ...
    /* Para strings (toContain sólo mira arrays). */
    toContainText(sub){
      if(typeof actual !== 'string' || actual.indexOf(sub) < 0)
        throw new Error('expected ' + fmt(actual) + ' to contain the text ' + fmt(sub));
    },
    toBeTruthy(){
